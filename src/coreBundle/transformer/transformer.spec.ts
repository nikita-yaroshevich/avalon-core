...
describe('transformers:', () => {
  ...
  it('should transform several values in Chain via FunctionTransformer and simple function', () => {
    ...
    const result = transformer.addTransformer((data:any[])=>{
  ...
  it('should transform several values in Chain via FunctionTransformer and simple function by TransformManager', () => {
    const transformer = new ChainTransformer(new CollectionTransformer(new FunctionTransformer(transformFunction)));
    let tm = new TransformManager();
    const result = tm.createData<any[]>(data, transformer.addTransformer((data:any[])=>{
      return data.map((d)=>d.name + '1');
    }));
  ...
  it('should transform several values with ItemTransformerAbstract and exclude email include email1stLvlDomain', () => {
    const result = new CollectionTransformer(new SimpleTransformerTest()).transform(data, {include: 'email1stLvlDomain', exclude: 'email'});
  ...
  it('ChainTransformer should remove transformer if it exist', () => {
    ...
    expect(transformer.hasTransformer(()=>{1})).toBeFalsy();
    expect(transformer.removeTransformer(transformFunction)).toBe(transformFunction);
    expect(transformer.getTransformers().length).toBe(2);
    expect(()=>transformer.removeTransformer(()=>{1})).toThrow();
  });
});
